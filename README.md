...
93.3 只保留为历史评分，不再用来证明融合版优于独立版。v1.1 是针对真实阅读反馈做的回归修复，尚未用新的主观分数替代用户验收。完整过程和修订后的样稿见 [评测说明](docs/evaluation.md) 与 [对照样稿](examples/evaluation/README.md)。
...
- `scripts/lint_ai_style.py` 提供 `universal`、`social-longform` 与公众号专用的 `wechat-longform` 三个检查档。
...
- 社媒严格档会清理部分标点、翻案句和黑话；公众号档额外拦截作者自设问句和表演性点题。README、PRD 和教程保留必要的表格、列表、代码与术语。
- 画像保存的是可迁移写作决策，不保存整篇样文，也不保证复刻某位作者。

## 验证

发布前执行了 Skill 结构校验、Python 语法检查、画像脚本空目录测试、三个文体检查档、公众号正反例回归和[一键安装冒烟测试](tests/smoke.sh)。检查命令与评测边界见 [评测说明](docs/evaluation.md)。
