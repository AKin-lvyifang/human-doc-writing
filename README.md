...
本项目融合并改编了 [KKKKhazix/human-writing](https://github.com/KKKKhazix/human-writing) 1.0.0 的方法，研究基线为提交 `22d20b672680e4c1a34e75aec550ff48d622ca59`。
...
融合过程使用三组相同材料做了匿名对照评审，覆盖 GPT-5.6 长任务、UI Skills 总入口和个人写作系统三个议题。

| 版本 | 三篇平均分 |
|---|---:|
| 独立 `human-writing` | 91.3 |
| 第一版融合 | 88.3 |
| 针对连续叙事与重复解释优化后的版本 | **93.3** |

第一版融合虽然通过质量门槛，却因固定小标题、重复定义和内部报告感落后于独立版。最终版只针对这组系统性问题迭代，三篇得分为 96、89、95，两个检查器的硬阻断项均为零。

这是一组内部前向测试，用来确认融合没有让写作退化，不是通用模型排行榜。完整方法、分项结果和样稿见 [评测说明](docs/evaluation.md) 与 [对照样稿](examples/evaluation/README.md)。
...
- `references/human-voice-gate.md` 负责所有文体共同的人味终审。
- `scripts/lint_ai_style.py` 提供 `universal` 与 `social-longform` 两个检查档。
...
- 社媒严格档会清理部分标点、翻案句和黑话；README、PRD 和教程保留必要的表格、列表、代码与术语。
- 画像保存的是可迁移写作决策，不保存整篇样文，也不保证复刻某位作者。

## 验证

发布前执行了 Skill 结构校验、Python 语法检查、画像脚本空目录测试、两个文体检查档和[一键安装冒烟测试](tests/smoke.sh)。检查命令与评测边界见 [评测说明](docs/evaluation.md)。
